...
def Jf_exact_(x):
	...
	if(x.imag == 0):
		x = abs(x)
		return integrate.quad(f, 0, integrate.inf)[0]
	else:
	#	x = abs(x) # line of doom!!! This takes the absolute value in both f and f1, when I only wanted it in f1
		f1 = lambda y: -y*y*log(2*abs(numpy.cos(sqrt(abs(x*x)-y*y)/2)))
		return integrate.quad(f1,0,abs(x))[0] + integrate.quad(f,abs(x),integrate.inf)[0]
		
def Jf_exact2_(theta): # Function of theta so that you can get negative values
	f = lambda y: -y*y*log(1+exp(-sqrt(y*y+theta))).real
	if theta >= 0:
		return integrate.quad(f, 0, integrate.inf)[0]
	else:
		f1 = lambda y: -y*y*log(2*abs(numpy.cos(sqrt(-theta-y*y)/2)))
		return integrate.quad(f, abs(theta)**.5, integrate.inf)[0] + integrate.quad(f1, 0, abs(theta)**.5)[0]
...
def Jb_exact2_(theta): # Function of theta so that you can get negative values
	f = lambda y: y*y*log(1-exp(-sqrt(y*y+theta))).real
	if theta >= 0:
		return integrate.quad(f, 0, integrate.inf)[0]
	else:
...
# This function allows a 1D array to be passed to something that normally can't handle it
...
Jf_exact = lambda x: arrayFunc(Jf_exact_, x, complex)
Jf_exact2 = lambda x: arrayFunc(Jf_exact2_, x)
Jb_exact = lambda x: arrayFunc(Jb_exact_, x)
Jb_exact2 = lambda x: arrayFunc(Jb_exact2_, x)
dJf_exact = lambda x: arrayFunc(dJf_exact_, x)
dJb_exact = lambda x: arrayFunc(dJb_exact_, x)
...
try:
	...
	# x = |xmin|*sinh(y), where y in linear (so that we're not overpopulating the uniteresting region)
	_xf = numpy.linspace(numpy.arcsinh(-1.3*20), numpy.arcsinh(-20*_xfmax/_xfmin), 1000)
...
# Spline fitting, Jb
_xbmin = -3.72402637 #-11.379714 # We're setting the lower acceptable bound as the point where it's a minimum
# This gaurantees that it's a monatonically increasing function, and the first deriv is continuous.
_xbmax = 1.41e3
try:
	...
	# x = |xmin|*sinh(y), where y in linear (so that we're not overpopulating the uniteresting region)
	_xb = numpy.linspace(numpy.arcsinh(-1.3*20), numpy.arcsinh(-20*_xbmax/_xbmin), 1000)
...
l = numpy.arange(50)+1
g = -2*pi**3.5 * (-1)**l*(1+special.zetac(2*l+1))*special.gamma(l+.5)/(fac(l+2)*(2*pi)**(2*l+4))
...
l = numpy.arange(50)+1
g = .25*pi**3.5 * (-1)**l*(1+special.zetac(2*l+1))*special.gamma(l+.5)*(1-.5**(2*l+1))/(fac(l+2)*pi**(2*l+4))
...
# Note that if approx = 'spline', the function called is J(theta) (x^2 -> theta so you can get negative mass squared)
def Jb(x, approx='high', deriv=0, n = 8):
	if(approx == 'exact'):
		if(deriv == 0):
			...
			raise Exception, "I've only programmed the first derivative for the exact integral."
	if(approx == 'spline'):
		return Jb_spline(x, deriv)
	if(approx == 'low'):
		if(n > 100):
			raise Exception, "n must be less than 100 for the low x approximation."
		if(deriv == 0):
			return Jb_low(x,n)
		else:
			raise Exception, "I haven't programmed any derivatives for the low-x approximation."
	if(approx == 'high'):
		if(deriv > 3):	
			raise Exception, "I've only programmed the first three derivatives for the high-x approximation."
		else:
