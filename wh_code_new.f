...
c
      ...
      NMAX=5000            !massimo numero di iterazioni
...
c
      DO 1 I=1,NP
      X=-LO+H*(I-1)
      CTMP=2.D0*MM+12.D0*LAMB*X*X + 6.D0*ASIM3*X
      UO(I)=DLOG(1.D0+CTMP*DEXP(2*T))
      Y=X+H/2.D0
      CHEM(I)=2.D0*MM*X+4.D0*LAMB*X*X*X + 3.D0*ASIM3*X*X*X+ASIM1
      OM(I)=MM*X**2+LAMB*X**4+ASIM3*X**3+ASIM1*X+COST
...
C    UPDATING OF CHEMICAL POTENTIAL AND FREE ENERGY
         ...
         DO I=1,NP-1
          ...
          OM(I)=OM(I)+0.25D0*(FATCHO*(UO(I))+FATCH*(U(I)))*K
c          OM(I)=OM(I)+0.25D0*(FATCHO*(UO(I)-2.D0*(T-K))+
c     &          FATCH*(U(I)-2.D0*T))*K
 ...
 40   CONTINUE
      ...
      X = -L1
      CTMP1 = 2.D0*MM + 12.D0*LAMB*L1*L1 - 6.D0*ASIM3*L1
      UO(1) = DLOG(1.D0 + CTMP1*DEXP(2*T))

      X = L1
      CTMP2 = 2.D0*MM + 12.D0*LAMB*L1*L1 + 6.D0*ASIM3*L1
      UO(NP) = DLOG(1.D0 + CTMP2*DEXP(2*T))

      
      IF(CONV.GE.1.E-6.OR.N.LT.ITERMIN) GO TO 10

 777  CONTINUE
      
      IDX_CHANGE = 0  ! Inizializzazione dell indice a 0 per indicare che non è stato trovato alcun cambio di segno

      DO I = 1, NP-1
          IF (CHEM(I) * CHEM(I+1) .LT. 0.0D0) THEN
              IDX_CHANGE = I
              EXIT  ! Esce dal ciclo una volta trovato il primo cambio di segno
          END IF
      END DO

      IF (IDX_CHANGE .NE. 0) THEN
          WRITE(*,*) 'Cambio di segno trovato allindice:', IDX_CHANGE
      ELSE
          WRITE(*,*) 'Nessun cambio di segno trovato nell array.'
      END IF

      DO  I=1,NP
             X=-L1+H*(I-1)
             WRITE(20,27)X,CHEM(I)
      END DO

      S1 = SQRT((EXP(-2.D0*T))*(DEXP(U(IDX_CHANGE)) - 1.0))
      ...
      DO 920 I=1,NP
             ...
             DERCH=(CHEM(I)-CHEM(I-1))/H
             CHINT=CHINT+PM*H*FINT
c             WRITE(20,601)x,PM,CHEM(I),U(I)
c             WRITE(20,27)X,DEV(I)
             FINT=0.5D0
             CHINT=CHINT+PM*H*FINT
 920    CONTINUE
